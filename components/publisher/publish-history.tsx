...
import { MessageSquare, CheckCircle, XCircle, Clock, Send, ArrowLeftRight, Zap } from "lucide-react"
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Recent messaging activity with responses</CardDescription>
          </div>
          {logs.length > 0 && (
                ...
                <div key={log.id} className="border rounded-lg p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className={`flex items-center gap-1 text-white ${getMessageTypeColor(log.messageType)}`}>
                  ...
                  {log.streamName && <div className="text-xs text-muted-foreground">Stream: {log.streamName}</div>}
                  {log.responseTopic && (
                    <div className="text-xs text-muted-foreground">Response Topic: {log.responseTopic}</div>
                  )}
                  ...
                  <details className="text-sm">
                    ...
                    <pre className="mt-2 p-2 bg-muted rounded text-xs font-mono whitespace-pre-wrap">{log.payload}</pre>
                  </details>
                </div>
              ))}
            </div>
