...
import { Send, Clock, AlertCircle, ArrowLeftRight, Zap } from "lucide-react"
...
export function MessagePublisher() {
  const { topics, globalVariables, isConnected, currentServer } = useNatsStore()
  const [selectedTopicId, setSelectedTopicId] = useState<string>("")
  const [customTopic, setCustomTopic] = useState("")
  const [customPayload, setCustomPayload] = useState("{}")
  const [customResponseTopic, setCustomResponseTopic] = useState("")
  const [isCustomMode, setIsCustomMode] = useState(false)
  const [customMessageType, setCustomMessageType] = useState<"publish" | "request" | "jetstream">("publish")
  ...
  const handlePublish = async () => {
    ...
    const messageType = isCustomMode ? customMessageType : selectedTopic?.messageType || "publish"
    const responseTopic = isCustomMode ? customResponseTopic : selectedTopic?.responseTopic
    ...
    const logEntry: PublishLog = {
      ...
      responseTopic: responseTopic
        ...
        : undefined,
      isJetStream: messageType === "jetstream" || (isCustomMode ? isJetStream : selectedTopic?.isJetStream || false),
      streamName: isCustomMode ? streamName : selectedTopic?.streamName,
      status: messageType === "request" ? "waiting" : "success",
    }

    try {
      const resolvedTopic = isCustomMode ? topic : resolvePayload(topic, localVariables)
      const resolvedPayload = isCustomMode ? payload : resolvePayload(payload, localVariables)
      ...
      if (messageType === "request") {
        ...
        // JetStream publish
        await natsClient.publishJetStream(resolvedTopic, resolvedPayload, logEntry.streamName)
        console.log("[v0] JetStream message published successfully")

        if (logEntry.responseTopic) {
          console.log("[v0] Listening for responses on:", logEntry.responseTopic)
        }
      } else {
        // Regular publish
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Publish messages, make requests, or send JetStream messages to NATS topics
              </CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex justify-end">
            <Button
              ...
              disabled={!isConnected || isPublishing || (!isCustomMode && !selectedTopic)}
            >
              {isPublishing ? (
                <>
