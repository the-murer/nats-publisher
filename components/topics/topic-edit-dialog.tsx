...
import { Eye, Send, ArrowLeftRight, Zap } from "lucide-react"
...
export function TopicEditDialog({ topic, onClose }: TopicEditDialogProps) {
  const { globalVariables, updateTopic } = useNatsStore()
  const [formData, setFormData] = useState<TopicFormData>({
    name: topic.name,
    topic: topic.topic,
    payload: topic.payload,
    variables: { ...topic.variables },
    messageType: topic.messageType || "publish",
    responseTopic: topic.responseTopic || "",
    isJetStream: topic.isJetStream,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const topicData = {
      ...
      isJetStream: formData.messageType === "jetstream" ? true : formData.isJetStream,
  ...
  const addVariable = () => {
    if (newVariableKey.trim() && newVariableValue.trim()) {
      setFormData({
        ...formData,
        variables: { ...formData.variables, [newVariableKey.trim()]: newVariableValue.trim() },
      })
      setNewVariableKey("")
      setNewVariableValue("")
    }
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>Update your NATS topic configuration</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: "publish" | "request" | "jetstream") =>
                  setFormData({ ...formData, messageType: value, isJetStream: value === "jetstream" })
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            {(formData.messageType === "publish" || formData.messageType === "jetstream") && (
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, responseTopic: e.target.value })}
                />
                {formData.responseTopic && getUsedVariables(formData.responseTopic).length > 0 && (
                  <div className="text-xs text-muted-foreground">
                    Preview: {resolveText(formData.responseTopic, formData.variables)}
                  </div>
                )}
            ...
            {formData.messageType === "jetstream" && (
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">JetStream stream name for persistent message storage</p>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              {previewMode ? (
                <div className="space-y-2">
                  <div className="text-sm text-muted-foreground">Resolved payload:</div>
                  <Textarea
                  ...
                  />
                  <div className="text-xs text-muted-foreground">
                    Variables used: {getUsedVariables(formData.payload).join(", ") || "None"}
                  </div>
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Use {"{{variableName}}"} syntax to insert variables. Example: {"{{userId}}"}, {"{{action}}"}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                Variables specific to this topic. Override global variables with the same name.
              </p>
