import { connect, type NatsConnection, type Msg, type JetStreamManager, type JetStreamClient } from "nats.ws"
...
export class NatsClient {
  ...
  async publishJetStream(subject: string, data: string | Uint8Array, streamName?: string): Promise<void> {
    if (!this.jetstream) {
    ...
    try {
      ...
      console.log("[v0] Published JetStream message:", { subject, stream: ack.stream, seq: ack.seq })
    } catch (error) {
  ...
  async subscribe(subject: string, callback: (msg: Msg) => void): Promise<void> {
    if (!this.connection) {
      throw new Error("Not connected to NATS server")
    }
